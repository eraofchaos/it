...
toc: true
---
 **Description:** Un fedele generale al servizio della famiglia reale dell'Erathia. Dalla morte di Re Cuor di Grifone, Morgan Kendal ha assunto il ruolo di sovrintendente, nell'attesa del ritorno del legittimo erede al trono.
## Details
 ...
 **Description:** Riduce Rid DAN ({-($buffaddattr12+$buffaddattr13*($sklevel-1))}<-($buffaddattr12+$buffaddattr13*($sklevel-1))*($a124+$a126+$a134+$a136)>%) e Schivata ({-($buffaddattr22+$buffaddattr23*($sklevel-1))/20}<-(($buffaddattr22+$buffaddattr23*($sklevel-1))/20)*($a124+$a126+$a134+$a136)>%) delle unità bersaglio per <span style="color: #48b946;font-size:20px">{($bufflast11+$bufflast12*($sklevel-1))/1000}</span><span style="color: black"> s.

### 4 Spell: Tattiche d'assalto
 **Description:** Aumenta Vel ATT e resistenza magica delle unità alleate nell'area d'effetto rispettivamente del 30% e 50% per <span style="color: #48b946;font-size:20px">{($olast11+$olast12*($sklevel-1))/1000}</span><span style="color: black"> sec. Effetto raddoppiato per le unità da 9 membri.
 ...
 **Description:** <span style="color: #ffffff;font-size:20px">L'Erathia è con me!</span>
